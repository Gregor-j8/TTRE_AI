...
def on_key(event):
    ...
    if event.key == 'enter' or event.key == ' ':
        ...
        if len(current_waypoints) == 0:
            ...
            route_waypoints[key] = [src, tgt]
            print(f"  Using straight line (no waypoints clicked)")
        else:
            route_waypoints[key] = current_waypoints.copy()
...
def show_current_route():
    ...
    ax.set_title(f"Route {current_route_idx + 1}/{len(routes)}: {src_name} -> {tgt_name} ({color}, {carriages} cars)\n"
    ...
    fig.canvas.draw()
    print(f"\nRoute {current_route_idx + 1}/{len(routes)}: {src_name} -> {tgt_name} ({color})")

def save_waypoints():
    with open('data/route_waypoints.json', 'w') as f:
        json.dump(route_waypoints, f, indent=2)
    print(f"Saved {len(route_waypoints)} routes to data/route_waypoints.json")
...
print("=== ROUTE WAYPOINT CALIBRATION ===")
print("Click points along each route (in order from source to target)")
print("Press ENTER/SPACE when done with a route (or press without clicking for straight line)")
