...
class TrainerV4:
    ...
    def load_v3_opponent(self, checkpoint_path):
        self.v3_opponent = V3Opponent(checkpoint_path, self.device)
    ...
    def _assign_game_type(self):
        ...
        if r < 0.25:
            return 'selfplay'
        elif r < 0.50 and self.v3_opponent:
            return 'v3'
        elif r < 0.70:
            return 'overall_game'
        elif r < 0.90:
    ...
    def _opponent_choose(self, game_type, game_state, player, legal_actions, board, player_idx):
        if game_type == 'v3' and self.v3_opponent:
            return self.v3_opponent.choose(game_state, player, legal_actions, board, player_idx)
        elif game_type == 'overall_game':
            return overall_game_choose(game_state, player, legal_actions, board)
        elif game_type == 'blitz':
            return blitz_choose(game_state, player, legal_actions, board)
        else:
    ...
    def collect_episodes(self, num_episodes):
        ...
        v4_player = [0] * self.batch_size

        for i in range(self.batch_size):
            game_type[i] = self._assign_game_type()
            v4_player[i] = i % 2
        ...
        while completed < num_episodes:
            for i, game in enumerate(games):
                ...
                if player_idx != v4_player[i]:
                    ...
                    action = self._opponent_choose(
                        game_type[i], game.state, player, actions, game.board, player_idx
                    )
            ...
            for i, game in enumerate(games):
                ...
                if game_type[i] != 'selfplay' and player_idx != v4_player[i]:
                    continue
                ...
                for i, game in enumerate(games):
                    if game.game_over or turns[i] >= 500:
                        ...
                        if game_type[i] != 'selfplay':
                            v4_traj = [[],[]]
                            v4_traj[v4_player[i]] = trajectories[i][v4_player[i]]
                            all_trajectories.append(v4_traj)
            ...
            for batch_idx, game_idx in enumerate(active_indices):
                ...
                if game.game_over or turns[game_idx] >= 500:
                    ...
                    if game_type[game_idx] != 'selfplay':
                        v4_traj = [[], []]
                        v4_traj[v4_player[game_idx]] = trajectories[game_idx][v4_player[game_idx]]
                        all_trajectories.append(v4_traj)
                        all_rewards.append(rewards)
                    else:
                    ...
                    if completed < num_episodes:
                        ...
                        turns[game_idx] = 0
                        game_type[game_idx] = self._assign_game_type()
                        v4_player[game_idx] = completed % 2
    ...
    def evaluate_vs_v3(self, num_games=100):
        if not self.v3_opponent:
            return None
        ...
        for i in range(num_games):
            ...
            while not game.game_over and turn < 500:
                ...
                if player_idx == model_player:
                    ...
                    action = self.v3_opponent.choose(game.state, player, actions, game.board, player_idx)
    ...
    def train(self, num_episodes=10000, eval_every=2000):
        print(f"Starting V4 RL training for {num_episodes} episodes...")
        print(f"Model parameters: {sum(p.numel() for p in self.model.parameters()):,}")
        print(f"Hyperparameters: lr={self.optimizer.param_groups[0]['lr']}, "
              f"entropy={self.entropy_coef} (decay={self.entropy_decay})")
        ...
        print("\n" + "=" * 60)
        print("TRAINING (25% Self-Play + 25% V3 + 20% Overall + 20% Blitz + 10% Heuristic)")
        print("=" * 60)
        ...
        while total_episodes < num_episodes:
            ...
            if total_episodes % eval_every < self.batch_size:
                ...
                print(f"  -> Rand={vs_random*100:.0f}%, Heur={vs_heuristic*100:.0f}%, "
                      f"Over={vs_overall*100:.0f}%, Blitz={vs_blitz*100:.0f}%{v3_str} ({elapsed/60:.1f}min)", flush=True)
                ...
                self.save(f"model_v4_ep{total_episodes}.pt")
